                ...
                <div class="col-md-12">
                  <div class="card">
                    <div class="card-header">
                      <h4>All Registered Company Users</h4>
                    </div>
                      ...
                      <div class="table-responsive">
                        ...
                        >
                          ...
                          <tbody>
                            ...
                            >
                              <td>{{ index + 1 }}</td>
                              <td>{{ provider.organisationName }}</td>
                              <td>{{ provider.companyPhoneNumber }}</td>
                              <td>{{ provider.email }}</td>
                              <td>{{ provider.userName }}</td>
                              <td v-if="provider.status === 1">
                                <span class="badge badge-success">Active</span>
                              </td>
                              <td v-if="provider.status === 0">
                                <span class="badge badge-danger">Deactive</span>
                              </td>
                              <td>{{ provider.country }}</td>
                              <td>{{ provider.created_at | customDate }}</td>
                              <td v-if="provider.status === 0">
                                <button
                                  @click="ActivateUser(provider)"
                                ...
                                |
                                <button
                                  @click="ProviderDetails(provider.id)"
...
export default {
  ...
  methods: {
    ...
    ProviderDetails(id) {
      ...
      window.location.href = '/company/or/provider/details/'+btoa(id)
    ...
    ActivateUser(user) {
      ...
      axios
        ...
        .then((response) => {
          Swal.fire({
            icon: "success",
            title: "Activating Provider Response",
            text: response.data.message,
          });
          window.location.href = "/manage/service/providers";
    ...
    DeActivateUser(user) {
      ...
      axios
        ...
        .then((response) => {
          Swal.fire({
            icon: "success",
            title: "DeActivating Provider Response",
            text: response.data.message,
          });
          window.location.href = "/manage/service/providers";
          $("#allCompanies").DataTable();
